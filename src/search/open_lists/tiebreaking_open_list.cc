...
template<class Entry>
typename Buckets<Entry>::iterator TieBreakingOpenList<Entry>::get_bucket(vector<int> *key) {
...
Entry TieBreakingOpenList<Entry>::remove_min(vector<int> *key) {
    assert(size > 0);
    --size;
    auto it = get_bucket(key);
    auto &bucket = it->second;
    
    Entry result = pop_bucket<Entry,Bucket<Entry>>(bucket, this->queue_type);
    if (bucket.empty()){
        buckets.erase(it);
    }
    return result;
